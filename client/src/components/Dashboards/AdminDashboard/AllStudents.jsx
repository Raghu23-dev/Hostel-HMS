...
function AllStudents() {
  // State for storing all students and search query
  const [allStudents, setAllStudents] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");

  // Function to handle CSV download
  const getCSV = async () => {
    // Get hostel ID from local storage
  ...
  const getAll = async () => {
    const data = await getAllStudents(); // Fetch data using utility function
    setAllStudents(data.students); // Update state with fetched students
  };
  ...
  return (
    <div className="w-full h-screen flex flex-col items-center justify-center ">
      {/* Search and CSV Download Section */}
        ...
        <div className="px-10 py-5 rounded-xl shadow-xl sm:w-[50%] sm:min-w-[500px] w-full mx-auto mt-5 max-h-[calc(100vh-140px)] overflow-auto transition-transform transform hover:scale-110 bg-gray-800">
          <ul role="list" className="divide-y divide-gray-700 text-white">
            {filteredStudents.length === 0 ?
              "No Students Found" // Message when no students match the search query
              :
